...
void onMQTTMessageReceivedPIDKP(const String& message){                     // Função para converter a string do MQTT para parâmetro Kp do controlador PID
  Kp.dFloat = message.toFloat();                                            // Converte o texto para float
  mbS.Hreg(ESPHREGPIDKP, Kp.dVec[1]);                                       // Armazena o valor de Kp no registrador correspondente
  mbS.Hreg(ESPHREGPIDKP+1, Kp.dVec[0]);                                     // Armazena o valor de Kp no registrador correspondente
  Serial.print("Kp: ");                                                     // Envia na serial o Kp recebido
  Serial.print(Kp.dFloat);                                                  // Envia na serial o Kp recebido
}                                                                           // Fim da Função*/

void onMQTTMessageReceivedPIDKI(const String& message){                     // Função para converter a string do MQTT para parâmetro Ki do controlador PID
  Ki.dFloat = message.toFloat();                                            // Converte o texto para float
  mbS.Hreg(ESPHREGPIDKI, Ki.dVec[1]);                                       // Armazena o valor de Ki no registrador correspondente
  mbS.Hreg(ESPHREGPIDKI+1, Ki.dVec[0]);                                     // Armazena o valor de Ki no registrador correspondente
  Serial.print("Ki: ");                                                     // Envia na serial o Ki recebido
  Serial.print(Ki.dFloat);                                                  // Envia na serial o Ki recebido
}

void onMQTTMessageReceivedPIDKD(const String& message){                     // Função para converter a string do MQTT para parâmetro Kd do controlador PID
  Kd.dFloat = message.toFloat();                                            // Converte o texto para float
  mbS.Hreg(ESPHREGPIDKD, Kd.dVec[1]);                                       // Armazena o valor de Kd no registrador correspondente
  mbS.Hreg(ESPHREGPIDKD+1, Kd.dVec[0]);                                     // Armazena o valor de Kd no registrador correspondente
  Serial.print("Kd: ");                                                     // Envia na serial o Kd recebido
  Serial.print(Kd.dFloat);                                                  // Envia na serial o Kd recebido
}
...
void onConnectionEstablished(){                                             // Função executada no estabelecimento da conexão mQTT
  ...
  //client.subscribe("/TkSys/W/PID/Kp", onMQTTMessageReceivedPIDKP);          // Assina o tópico "/TkSys/W/PID/Kp" para receber o parâmetro Kp do PID via MQTT
  //client.subscribe("/TkSys/W/PID/Ki", onMQTTMessageReceivedPIDKI);          // Assina o tópico "/TkSys/W/PID/Ki" para receber o parâmetro Ki do PID via MQTT
  //client.subscribe("/TkSys/W/PID/Kd", onMQTTMessageReceivedPIDKD);          // Assina o tópico "/TkSys/W/PID/Kd" para receber o parâmetro kd do PID via MQTT
  ...
  //client.publish("/TkSys/R/PID/Kp", String(Kp.dFloat,2));        // Publica uma mensagem inicial para o tópico "/TkSys/R/PID/Kp"
  //client.publish("/TkSys/R/PID/Ki",String(Ki.dFloat,2));        // Publica uma mensagem inicial para o tópico "/TkSys/R/PID/Ki"
  //client.publish("/TkSys/R/PID/Kd", String(Kd.dFloat,2));        // Publica uma mensagem inicial para o tópico "/TkSys/R/PID/Kd"
...
// ############################################################################## SLAVE #############################################################################################
    if (!ucs--) {                                                           // Requisita dados a cada 1 segundo (seguindo as configurações)
      ...
      if(xSemaphoreTake(mutexV, pdMS_TO_TICKS(updateTimeMsMBMS/2))){        // Tenta pegar o mutex da tensão pelo periodo máximo da metade do tempo de atualização da task
        ...
        //Kp.dVec[1] = mbS.Hreg(ESPHREGPIDKP);                                
        //Kp.dVec[0] = mbS.Hreg(ESPHREGPIDKP+1);                              // Converte o valor do registrador de amazenamento do Kp para float
        //Ki.dVec[1] = mbS.Hreg(ESPHREGPIDKI);                                
        //Ki.dVec[0] = mbS.Hreg(ESPHREGPIDKI+1);                              // Converte o valor do registrador de amazenamento do Ki para float
        //Kd.dVec[1] = mbS.Hreg(ESPHREGPIDKD);                                
        //Kd.dVec[0] = mbS.Hreg(ESPHREGPIDKD+1);                              // Converte o valor do registrador de amazenamento do Kd para float
                                                                              
        //client.publish("/TkSys/R/PID/Kp", String(Kp.dFloat,6));  // Publica o valor do Kp para o tópico "/TkSys/R/PID/Kp"
        //client.publish("/TkSys/R/PID/Ki", String(Ki.dFloat,6));  // Publica o valor do Ki para o tópico "/TkSys/R/PID/Ki"
        //client.publish("/TkSys/R/PID/Kd", String(Kd.dFloat,6));  // Publica o valor do Kd para o tópico "/TkSys/R/PID/Kd"
